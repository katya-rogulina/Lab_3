...
Метод `String toString()` возвращает название блюда и количество оставшихся шагов
`public int getSteps() {
    return steps; }` - геттер, который возвращает значение приватного поля steps из объекта. 
...
```
Метод `add(String name, int steps)` добавляет заказ с названием и количеством этапов
Метод `printActiveZakazList()` возвращает список активных заказов
Метод `oneAction(int index)` выполняет один этап в заказе, и в случае завершениея переносит заказ из списка активных в выполненные
Метод `nActions(int num, int n)` выполняет n действий в заказе
Метод `nActionsBeginning(int n)` выполняет n действий с начала списка, и также переносит выполненные заказы в соответствующий список
Метод `oneActionMany(int []indeces)` выполняет по одному этапу в каждом заказе, перечисленных в указанном массиве
Метод `printFinishedZakazList()` возвращает список выполненных заказов
Метод `cancelZakaz(int number)` отменяет заказ (удаляет его)
Метод `deleteZakaz(int index)` дополнительный метод для удаления заказа из списка активных и добавление его в список выполненных, при условии его завершения
